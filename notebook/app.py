...
live_display = (
    filtered_data[live_display_columns]
    .head(20)
    .copy()
)

live_display["Sanction Date"] = (
    live_display["Sanction Date"]
    .dt.strftime("%d-%b-%Y")
    .fillna("N/A")
)

live_display["Completion Date"] = (
    live_display["Completion Date"]
    .dt.strftime("%d-%b-%Y")
    .fillna("N/A")
)

live_display = live_display.rename(
    columns={
        "State_x": "State",
...
if search_id:
    ...
    if not result.empty:
        ...
        st.write(
            "**Progress:**",
            result["progress"].iloc[0]
        )

        sanction_date = result["Sanction Date"].iloc[0]

        if pd.isna(sanction_date):
            sanction_display = "N/A"
        else:
            sanction_display = sanction_date.strftime("%d-%b-%Y")
        ...
        completion_date = result["Completion Date"].iloc[0]

        if pd.isna(completion_date):
            completion_display = "Not Completed"
        else:
            completion_display = completion_date.strftime("%d-%b-%Y")
